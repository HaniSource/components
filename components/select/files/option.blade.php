...
<li class="contents col-span-full" x-show="isItemShow(@js($value))">
    <button x-on:mouseenter="$el.focus()" x-bind:value="@js($value)" type="button" variant="none"
        x-on:click="select(@js($value))"
        :class="{
            'bg-neutral-200 dark:bg-neutral-800 [&>[data-slot=icon]]:opacity-100': isSelected(@js($value))
        }"
        class="focus:bg-neutral-100 focus:dark:bg-neutral-700 px-3 py-1  rounded-[calc(var(--round)-var(--padding))] col-span-full grid grid-cols-subgrid w-full gap-x-2">
